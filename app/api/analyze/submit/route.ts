...
async function handleSubmit(request: NextRequest) {
  ...
  if (!isValidUrl(url)) {
    return ApiResponses.badRequest('Invalid URL format. Please include http:// or https://');
  }
  ...
  if (recentAnalysis) {
    return ApiResponses.tooManyRequests('Analysis for this URL was recently requested. Please wait before requesting again.');
  }
  ...
  fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/analyze/process`, {
    method: 'POST',
    headers: {
