...
import { extractWebsiteData, closeExtractionEngine } from '@/lib/dataExtraction';
import { analyzePerformance, PerformanceAnalysis } from '@/lib/analyzers/performance';
...
import { sendAnalysisCompleteEmail, sendAnalysisFailedEmail } from '@/lib/email';
...
function generatePerformanceInsights(performanceResult: PerformanceAnalysis): string {
...
function generatePerformanceRecommendations(performanceResult: PerformanceAnalysis): string[] {
  const recommendations: string[] = [];
  
  // Core Web Vitals specific recommendations
  if (performanceResult.coreWebVitals.lcp > 2500) {
    recommendations.push('Optimize images: Use WebP format, add width/height attributes, and implement lazy loading');
    recommendations.push('Improve server response time: Use a CDN, optimize database queries, and enable compression');
  ...
  if (performanceResult.coreWebVitals.fid > 100) {
    recommendations.push('Reduce JavaScript execution time: Split large bundles and remove unused code');
    recommendations.push('Use a web worker for heavy computations to keep the main thread responsive');
    recommendations.push('Minimize main thread work: Defer non-essential JavaScript until after page load');
  }
  ...
  if (performanceResult.coreWebVitals.cls > 0.1) {
    ...
    recommendations.push('Reserve space for ads and embeds with CSS min-height properties');
    recommendations.push('Avoid inserting content above existing content unless in response to user interaction');
  }
  
  // Lighthouse opportunities-based recommendations
  if (performanceResult.opportunities && performanceResult.opportunities.length > 0) {
    performanceResult.opportunities.slice(0, 5).forEach(opportunity => {
      switch (opportunity.id) {
        case 'unused-css-rules':
          recommendations.push('Remove unused CSS: Use tools like PurgeCSS to eliminate unused styles');
          break;
        case 'unused-javascript':
          recommendations.push('Remove unused JavaScript: Use code splitting and tree shaking to reduce bundle size');
          break;
        case 'modern-image-formats':
          recommendations.push('Serve images in next-gen formats: Use WebP or AVIF instead of JPEG/PNG');
          break;
        case 'offscreen-images':
          recommendations.push('Defer offscreen images: Implement lazy loading for images below the fold');
          break;
        case 'render-blocking-resources':
          recommendations.push('Eliminate render-blocking resources: Inline critical CSS and defer JavaScript');
          break;
        case 'unminified-css':
          recommendations.push('Minify CSS: Use build tools to remove whitespace and comments from stylesheets');
          break;
        case 'unminified-javascript':
          recommendations.push('Minify JavaScript: Use build tools to compress and optimize JavaScript files');
          break;
        case 'efficient-animated-content':
          recommendations.push('Use video formats for animated content: Replace GIFs with MP4 or WebM videos');
          break;
        case 'duplicated-javascript':
          recommendations.push('Remove duplicate modules: Consolidate shared code to reduce bundle size');
          break;
        default:
  ...
  if (performanceResult.metrics.firstContentfulPaint > 1800) {
    recommendations.push('Improve First Contentful Paint: Optimize critical rendering path and reduce server response time');
  }
  
  if (performanceResult.metrics.timeToInteractive > 3800) {
    recommendations.push('Reduce Time to Interactive: Minimize JavaScript execution and optimize third-party scripts');
  }
  
  if (performanceResult.metrics.totalBlockingTime > 200) {
    recommendations.push('Reduce Total Blocking Time: Break up long tasks and optimize JavaScript execution');
  }
  
  // Ensure we have at least some recommendations
  if (recommendations.length === 0) {
    if (performanceResult.score >= 90) {
      recommendations.push('Excellent performance! Monitor Core Web Vitals and maintain current optimization levels');
      recommendations.push('Consider implementing performance budgets to prevent regression');
    } else {
      recommendations.push('Enable compression (Gzip/Brotli) to reduce file sizes');
      recommendations.push('Optimize images: Compress and use appropriate formats for better loading times');
      recommendations.push('Minimize HTTP requests by combining CSS and JavaScript files where possible');
    }
  }
...
async function handleProcess(request: NextRequest) {
  ...
  try {
    ...
    const analysisRequest = await DatabaseOperations.getAnalysisRequest(requestId);

    // Check if already processing or completed
    if (analysisRequest.status !== 'pending') {
      return ApiResponses.badRequest(`Analysis already ${analysisRequest.status}`);
    }
    ...
    if (error instanceof Error && error.message === 'Analysis request not found') {
...
async function processAnalysis(analysisRequest: AnalysisRequest) {
  ...
  try {
    ...
    const performanceResult = await analyzePerformance(analysisRequest.url, extractedData);

    // Store performance results
    const performanceInsights = generatePerformanceInsights(performanceResult);
    const performanceRecommendations = generatePerformanceRecommendations(performanceResult);
    ...
    const designResult = await analyzeDesign(extractedData, performanceResult, screenshotBase64);
    ...
    const responsivenessResult = await analyzeResponsiveness(extractedData, screenshotBase64);
    ...
    const complianceResult = await analyzeCompliance(analysisRequest.url, extractedData, screenshotBase64);
    ...
    const results = await DatabaseOperations.getAnalysisResults(analysisRequest.id);
    ...
    await DatabaseOperations.updateAnalysisStatus(analysisRequest.id, 'completed');

    // Send completion email
    const pillars = results?.reduce((acc, result) => {
      acc[result.pillar] = {
        score: result.score,
        analyzed: result.analyzed,
        insights: result.insights,
        recommendations: result.recommendations,
        ...(result.pillar === 'performance' && { grade: scoreToGrade(result.score) })
      };
      return acc;
    }, {} as PillarResults) || {};
    ...
    // Send failure email
    console.log(`📧 Attempting to send failure email to: ${analysisRequest.email} for URL: ${analysisRequest.url}`);
    await sendAnalysisFailedEmail(analysisRequest.url, analysisRequest.email, error instanceof Error ? error.message : 'Unknown error');
