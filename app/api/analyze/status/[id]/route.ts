...
async function handleGetStatus(
  ...
  if (idError) {
    ...
    // Calculate progress
    const totalPillars = 6; // Performance, Design, Responsiveness, SEO, Security, Compliance
    const completedPillars = (analysisRequest.analysis_results as AnalysisResult[])?.filter(
    ...
    const response = {
      ...
      pillars: (analysisRequest.analysis_results as AnalysisResult[])?.reduce((acc: PillarStatus, result) => {
        acc[result.pillar] = {
          score: result.score,
      ...
      estimatedTimeRemaining: analysisRequest.status === 'processing' 
        ? Math.max(0, 300 - Math.floor((Date.now() - new Date(analysisRequest.created_at).getTime()) / 1000))
        : 0
    };
