...
export async function analyzeDesign(
  ...
  performanceData?: { coreWebVitals?: { cls?: number } },
  ...
  const aiInsights = precomputedVision
    ? {
        primaryCTA: precomputedVision.primaryCTA,
        visualStyle: precomputedVision.visualStyle,
        designIssues: precomputedVision.designIssues,
        visionRecommendations: precomputedVision.recommendations,
      }
    : await generateAIInsights(extractedData, screenshotBase64);
  ...
  if (colorScheme.colorCount > 10) {
    score -= 15;
    issues.push(
      "Too many colors used - consider simplifying the color palette"
    );
    recommendations.push(
      "Limit your color palette to 3-5 main colors for better visual consistency"
    );
  }
  ...
  if (colorScheme.contrastRatio < 4.5) {
    ...
    recommendations.push(
      "Improve color contrast to meet WCAG AA standards (4.5:1 ratio minimum)"
    );
  }
  ...
  if (typography.fontFamilies.length > 3) {
    ...
    recommendations.push(
      "Limit to 2-3 font families maximum for better typography consistency"
    );
  }
  ...
  if (typography.readabilityScore < 70) {
    ...
    recommendations.push(
      "Increase font sizes and line height for better readability"
    );
  }
  ...
  if (layout.viewportUtilization < 60) {
    score -= 10;
    issues.push(
      "Poor viewport utilization - content appears cramped or poorly distributed"
    );
    recommendations.push(
      "Optimize layout to better utilize available screen space"
    );
  }
  ...
  if (extractedData.headings.filter((h) => h.level === 1).length !== 1) {
    recommendations.push(
      "Use exactly one H1 tag per page for better content hierarchy"
    );
  }

  // Add vision-based recommendations if available
  if (
    aiInsights.visionRecommendations &&
    aiInsights.visionRecommendations.length > 0
  ) {
  ...
  "recommendations": ["specific, actionable design recommendations"]
      ...
      if (content) {
        ...
        if (jsonMatch) {
          ...
          return {
            primaryCTA:
              visionResult.primaryCTA ||
              "No clear primary call-to-action identified",
...
function extractFontData(html: string) {
  ...
  const fontSizeMatches = html.match(
    /font-size:\s*(\d+(?:\.\d+)?)(px|em|rem|%)/gi
  );
  if (fontSizeMatches) {
...
function calculateReadabilityScore(fontData: {
  families: string[];
  sizes: number[];
}): number {
  let score = 100;
...
function identifyPrimaryCTA(extractedData: ExtractedData): string {
  ...
  const links = extractedData.links.filter(
    (link) =>
...
function identifyDesignIssues(extractedData: ExtractedData): string[] {
  ...
  const imagesWithoutAlt = extractedData.images.filter(
