...
export async function analyzeCompliance(
  ...
  const gdprCompliant = checkGDPRCompliance(
    extractedData,
    cookieConsent,
    privacyPolicy
  );
  if (!gdprCompliant) {
    ...
    recommendations.push(
      "Ensure GDPR compliance with proper consent mechanisms"
    );
  }
  ...
  const wcagLevel =
    accessibilityScore.score >= 80
      ? "AA"
      : accessibilityScore.score >= 60
      ? "A"
      : "Fail";
  ...
  // Use precomputed vision data if available, otherwise enhance with vision analysis
  if (precomputedVision) {
    // Use precomputed vision analysis
    const enhancedInsights = `Good compliance with room for improvement. ${
      precomputedVision.accessibility?.colorContrast === "good"
        ? "✅ Good color contrast detected."
        : "⚠️ Color contrast may need improvement."
    } ${
      precomputedVision.accessibility?.textReadability === "good"
        ? "✅ Text is readable and well-sized."
        : "⚠️ Text readability could be improved."
    } ${
      baseResult.wcagLevel === "AA"
        ? "✅ Meets WCAG 2.1 AA standards."
        : "⚠️ Consider improving accessibility standards."
    }`;
    ...
    return {
      ...
      recommendations: [
        ...
        ...(precomputedVision.recommendations || []).slice(0, 3),
      ],
    };
  } else if (screenshotBase64) {
    try {
      const enhancedResult = await enhanceComplianceWithVision(
        baseResult,
        screenshotBase64,
        url
      );
...
function checkCookieConsent(extractedData: ExtractedData): boolean {
  ...
  const hasButtons = buttonKeywords.some(
    (keyword) => html.includes(`>${keyword}<`) || html.includes(`"${keyword}"`)
  );
  const hasFrameworks = cookieFrameworks.some((framework) =>
...
function checkPrivacyPolicy(extractedData: ExtractedData): boolean {
  ...
  const privacyLinks = links.filter(
    (link) =>
...
function analyzeAccessibility(extractedData: ExtractedData): {
  score: number;
  issues: string[];
  recommendations: string[];
} {
